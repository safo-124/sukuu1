        ...
        : _error != null
          ...
          : ListView.builder(
              ...
              itemBuilder: (context, idx) {
                ...
                final name = ((c['firstName'] ?? '').toString() + ' ' + (c['lastName'] ?? '').toString()).trim();
                final promos = (c['promotions'] as List?) ?? [];
                final current = c['current'] as Map<String, dynamic>?;
                return ExpansionTile(
                  title: Text(name.isEmpty ? c['studentId']?.toString() ?? 'Student' : name),
                  subtitle: current == null
                    ? const Text('No current enrollment')
                    : Text('Current: ${current['academicYear'] ?? ''} • ${current['className'] ?? ''} • ${current['sectionName'] ?? ''}'),
                  children: promos.isEmpty
                    ? [const ListTile(title: Text('No promotion/transfer history'))]
                    : promos.map<Widget>((p) {
                        final mp = p as Map<String, dynamic>;
                        final type = (mp['type'] ?? '').toString();
                        final date = (mp['date'] ?? '').toString();
                        final from = (mp['from'] as Map<String, dynamic>?);
                        final to = (mp['to'] as Map<String, dynamic>?);
                        return ListTile(
                          leading: Icon(type == 'PROMOTED' ? Icons.trending_up : Icons.swap_horiz),
                          title: Text(type == 'PROMOTED' ? 'Promoted' : 'Transferred'),
                          subtitle: Text('${from?['academicYear'] ?? ''} • ${from?['className'] ?? ''} • ${from?['sectionName'] ?? ''}  →  ${to?['academicYear'] ?? ''} • ${to?['className'] ?? ''} • ${to?['sectionName'] ?? ''}\n$date'),
                        );
                      }).toList(),
                );
              },
