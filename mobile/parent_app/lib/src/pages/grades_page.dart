...
class _GradesPageState extends State<GradesPage> {
      ...
      setState(() {
        ...
        if (widget.allowChildSwitch) {
          final kids = children.cast<Map<String, dynamic>>().map((c) {
            final first = c['firstName'] ?? (c['student'] is Map ? c['student']['firstName'] : null);
            final last = c['lastName'] ?? (c['student'] is Map ? c['student']['lastName'] : null);
            return {
          ...
          if (_activeStudentId == null && kids.isNotEmpty) {
            _activeStudentId = (kids.first['id'] ?? widget.studentId).toString();
          }
        }
      });
  ...
  Future<void> _loadChildren() async {
    try {
      ...
      final kids = (meJson['children'] as List? ?? []).cast<Map<String, dynamic>>();
      setState(() {
        _children = kids;
        if (_activeStudentId == null && kids.isNotEmpty) {
          final first = kids.first;
          _activeStudentId = first['id']?.toString();
          _activeStudentName = ('${first['firstName'] ?? ''} ${first['lastName'] ?? ''}').trim();
        }
      });
    } catch (_) {}
  }
  ...
  List<Map<String, dynamic>> _applyFilters() {
    return _records.where((g) {
      final ayOk = _selectedAy == 'all' || (g['academicYear'] as Map?)?['id']?.toString() == _selectedAy;
      final termOk = _selectedTerm == 'all' || (g['term'] as Map?)?['id']?.toString() == _selectedTerm;
      return ayOk && termOk;
    }).toList();
  }
  ...
  Map<String, dynamic> _computeSummary(List<Map<String, dynamic>> list) {
    ...
    for (final g in list) {
      ...
      if (marks != null) {
        sumMarks += marks;
        countMarks += 1;
        minMarks = (minMarks == null) ? marks : (marks < minMarks ? marks : minMarks);
        maxMarks = (maxMarks == null) ? marks : (marks > maxMarks ? marks : maxMarks);
        if (maxM != null && maxM > 0) {
  ...
  Map<String, Map<String, dynamic>> _computeTermSummaries(List<Map<String, dynamic>> list) {
    // key: termName
  ...
  double bestAvg = -1.0;
      bySubject.forEach((k, v) {
        final double avg = v.isEmpty ? 0.0 : v.reduce((a, b) => a + b) / v.length;
        if (avg > bestAvg) {
          bestAvg = avg;
          topSubject = k;
        }
  ...
  Future<void> _exportPdf(List<Map<String, dynamic>> filtered, String studentName) async {
      ...
      pw.MultiPage(
        ...
        build: (ctx) {
          return [
              ...
              children: [
                ...
                pw.Expanded(
                  child: pw.Column(
                    ...
                    children: [
                      pw.Text('Grades Report', style: pw.TextStyle(fontSize: 20, fontWeight: pw.FontWeight.bold)),
                      pw.Text(studentName, style: const pw.TextStyle(fontSize: 12)),
                    ],
                  ),
                ),
              ],
            ...
            pw.Text('Filters: Year=${_selectedAy == 'all' ? 'All' : _selectedAy}, Term=${_selectedTerm == 'all' ? 'All' : _selectedTerm}', style: const pw.TextStyle(fontSize: 10)),
            pw.SizedBox(height: 12),
            if (termSummaries.isNotEmpty) ...[
              pw.Text('Term Summary', style: pw.TextStyle(fontSize: 14, fontWeight: pw.FontWeight.bold)),
              ...
              pw.Table(
                ...
                children: [
                  pw.TableRow(children: [
                    ...
                    pw.Padding(padding: const pw.EdgeInsets.all(4), child: pw.Text('Avg', style: pw.TextStyle(fontWeight: pw.FontWeight.bold))),
                    pw.Padding(padding: const pw.EdgeInsets.all(4), child: pw.Text('Min/Max', style: pw.TextStyle(fontWeight: pw.FontWeight.bold))),
                    pw.Padding(padding: const pw.EdgeInsets.all(4), child: pw.Text('Top Subject', style: pw.TextStyle(fontWeight: pw.FontWeight.bold))),
                  ]),
                  ...termSummaries.entries.map((e) {
                    ...
                    return pw.TableRow(children: [
                      ...
                      pw.Padding(padding: const pw.EdgeInsets.all(4), child: pw.Text(avg != null ? avg.toStringAsFixed(1) : '-')),
                      pw.Padding(padding: const pw.EdgeInsets.all(4), child: pw.Text('${min?.toStringAsFixed(1) ?? '-'} / ${max?.toStringAsFixed(1) ?? '-'}')),
                      pw.Padding(padding: const pw.EdgeInsets.all(4), child: pw.Text(top)),
                    ]);
                  })
                ],
              ),
            ...
            ...groups.map((grp) {
              ...
              return pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
                pw.Text(subject, style: pw.TextStyle(fontSize: 14, fontWeight: pw.FontWeight.bold)),
                pw.SizedBox(height: 6),
                pw.Table(
                  columnWidths: const {
                  ...
                  children: [
                    pw.TableRow(
                      ...
                      children: [
                        pw.Padding(padding: const pw.EdgeInsets.all(4), child: pw.Text('Exam', style: pw.TextStyle(fontWeight: pw.FontWeight.bold))),
                        ...
                        pw.Padding(padding: const pw.EdgeInsets.all(4), child: pw.Text('Band', style: pw.TextStyle(fontWeight: pw.FontWeight.bold))),
                        pw.Padding(padding: const pw.EdgeInsets.all(4), child: pw.Text('Remarks', style: pw.TextStyle(fontWeight: pw.FontWeight.bold))),
                      ],
                    ),
                    ...rows.map((g) {
                      final exam = (((g['examSchedule'] as Map?)?['exam']) as Map?)?['name']?.toString() ?? 'Exam';
                      ...
                      return pw.TableRow(children: [
                        pw.Padding(padding: const pw.EdgeInsets.all(4), child: pw.Text(exam)),
                        pw.Padding(padding: const pw.EdgeInsets.all(4), child: pw.Text(date != null ? df.format(DateTime.parse(date)) : '')),
                        pw.Padding(padding: const pw.EdgeInsets.all(4), child: pw.Text(marksStr)),
                        pw.Padding(padding: const pw.EdgeInsets.all(4), child: pw.Text(band)),
                        pw.Padding(padding: const pw.EdgeInsets.all(4), child: pw.Text(comments)),
                      ]);
                    })
                  ],
                ),
            ...
            if (groups.isEmpty) pw.Text('No grades found for the selected filters.')
          ];
        },
      ),
    );
                    ...
                    children: [
                                ...
                                child: DropdownButtonFormField<String>(
                                  ...
                                  onChanged: (v) {
                                    final sel = _children.firstWhere(
                                        ...
                                        orElse: () => {});
                                    setState(() {
                                      _activeStudentId = sel.isEmpty ? null : sel['id'].toString();
                          ...
                          children: [
                            ...
                            Expanded(
                              child: DropdownButtonFormField<String>(
                                ...
                                items: _buildTermOptions(_records.where((g) => _selectedAy == 'all' || (g['academicYear'] as Map?)?['id']?.toString() == _selectedAy).toList())
                                    .map((o) => DropdownMenuItem(value: o['id']!, child: Text(o['name']!)))
                                    .toList(),
                                onChanged: (v) => setState(() {
                                  _selectedTerm = v ?? 'all';
                      ...
                      Padding(
                        ...
                        child: Wrap(
                          ...
                          runSpacing: 8,
                          children: [
                            _StatChip(label: 'Items', value: '${summary['count']}'),
                            if (summary['avg'] != null)
                              _StatChip(label: 'Avg', value: (summary['avg'] as double).toStringAsFixed(1)),
                            if (summary['avgPct'] != null)
                              _StatChip(label: 'Avg %', value: '${(summary['avgPct'] as double).toStringAsFixed(1)}%'),
                            if (summary['min'] != null)
                              _StatChip(label: 'Min', value: (summary['min'] as double).toStringAsFixed(1)),
                            if (summary['max'] != null)
                              _StatChip(label: 'Max', value: (summary['max'] as double).toStringAsFixed(1)),
                          ],
                        ),
                      ),
                            ...
                            child: Padding(
                              ...
                              child: Column(
                                ...
                                children: [
                                  Row(
                                    children: [
                                      Expanded(child: Text(subject, style: const TextStyle(fontWeight: FontWeight.w600))),
                                      Container(
                                  ...
                                  ...rows.map((g) {
                                    ...
                                    final marksStr = g['marksObtained']?.toString() ?? '-';
                                    final maxMarks = ((g['examSchedule'] as Map?)?['maxMarks']);
                                    final term = (g['term'] as Map?)?['name']?.toString() ?? '';
                                    final year = (g['academicYear'] as Map?)?['name']?.toString() ?? '';
                                    double? marksNum = double.tryParse(marksStr);
                                    if (marksNum == null && g['marksObtained'] is num) {
                                      marksNum = (g['marksObtained'] as num).toDouble();
                                    }
                                        ...
                                        children: [
                                          ListTile(
                                            ...
                                            trailing: Column(
                                              ...
                                              children: [
                                                Text(marksStr, style: const TextStyle(fontWeight: FontWeight.bold)),
                                                if (band != null)
                                                  Text(band, style: TextStyle(color: color, fontSize: 11)),
                                              ],
                                            ),
                                          ),
                                          if (marksNum != null && maxMarks is num && maxMarks > 0)
                                            LinearProgressIndicator(
                                              value: (marksNum / (maxMarks.toDouble())).clamp(0.0, 1.0),
                                              minHeight: 6,
                                              color: color,
                      ...
                      if (groups.isEmpty)
                        const Padding(
                          ...
                          child: Center(child: Text('No grades found for the selected filters.')),
                        ),
                    ],
                  ),
                );
      ...
      return Scaffold(
        appBar: AppBar(
          ...
          actions: [
            ...
            IconButton(
              tooltip: 'Export PDF',
              onPressed: _loading ? null : () => _exportPdf(_applyFilters(), widget.studentName),
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          Text(value, style: TextStyle(color: Theme.of(context).colorScheme.primary, fontWeight: FontWeight.w600)),
        ],
      ),
    );
  }
