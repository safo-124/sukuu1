...
import { FilePlus2, Edit3, Trash2, BookOpen, AlertTriangle, UserCog, Loader2, AlertTriangleIcon } from 'lucide-react'; // Added UserCog for teacher select
import {
...
const SubjectFormFields = ({ formData, onFormChange, onSelectChange, departmentsList, teachersList, isLoadingDeps, isEdit = false }) => {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4 max-h-[60vh] overflow-y-auto p-1 custom-scrollbar">
      ...
      <div>
        <Label htmlFor={isEdit ? "editTeacherId" : "teacherId"} className={labelTextClasses}>Assign Teacher <span className="text-red-500">*</span></Label>
        <Select name="teacherId" value={formData.teacherId} onValueChange={(value) => onSelectChange('teacherId', value)} disabled={isLoadingDeps}>
          ...
          <SelectContent className="bg-white dark:bg-zinc-900">
            {teachersList?.length === 0 && <SelectItem value="loading" disabled>Loading teachers...</SelectItem>}
      ...
      <div className="sm:col-span-2">
        ...
        <Textarea id={isEdit ? "editDescription" : "description"} name="description" value={formData.description} onChange={onFormChange} rows={3} className={`${inputTextClasses} mt-1`} />
      </div>
      <div>
        ...
        <Select name="departmentId" value={formData.departmentId || 'none'} onValueChange={(value) => onSelectChange('departmentId', value)} disabled={isLoadingDeps}>
          ...
          <SelectContent className="bg-white dark:bg-zinc-900">
            <SelectItem value="none" className="text-zinc-500">No Department</SelectItem>
            {departmentsList?.map(dept => <SelectItem key={dept.id} value={dept.id}>{dept.name}</SelectItem>)}
          </SelectContent>
        </Select>
      </div>
    </div>
...
export default function ManageSubjectsPage() {
  ...
  const [isLoadingDeps, setIsLoadingDeps] = useState(true); // For dropdown dependencies
  ...
  // TODO: Add state and logic for Edit Dialog
  ...
  const fetchSubjects = useCallback(async () => { /* ... remains same ... */ 
    ...
    try {
      ...
      toast.error("Error fetching subjects", { description: err.message }); setError(err.message);
  ...
  const fetchDropdownDependencies = useCallback(async () => {
    ...
    try {
      const [deptRes, teacherRes] = await Promise.all([
        ...
        fetch(`/api/schools/${schoolData.id}/staff/teachers`) // ✨ Fetch teachers
      ]);
      ...
      setTeachers(teacherData.teachers || []); // ✨ Set teachers state

    } catch (err) {
      toast.error("Error fetching form data", { description: err.message });
    } finally {
  ...
  const openAddDialog = () => { setFormData(initialFormData); setFormError(''); setIsAddDialogOpen(true); };

  const handleAddSubmit = async (e) => {
    e.preventDefault();
    if (!schoolData?.id) return;
    if (!formData.teacherId) { // Client-side check for teacherId
        toast.error("Validation Error", { description: "A teacher must be selected." });
        setFormError("A teacher must be selected.");
        return;
    }
    setIsSubmitting(true); setFormError('');
    const dataToSubmit = { ...formData, departmentId: formData.departmentId || null, subjectCode: formData.subjectCode || null };
    ...
    try {
      ...
      if (!response.ok) {
        let err = result.error || 'Failed to create subject.';
        if(result.issues) err = result.issues.map(i => `${i.path.join('.') || 'Field'}: ${i.message}`).join('; ');
        toast.error("Creation Failed", { description: err }); setFormError(err);
      } else {
        toast.success(`Subject "${result.subject?.name}" created successfully!`);
        setIsAddDialogOpen(false); fetchSubjects();
      }
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
          ...
          <DialogContent className="sm:max-w-lg md:max-w-xl bg-white dark:bg-zinc-950 border-zinc-200 dark:border-zinc-800">
            <DialogHeader> <DialogTitle className={titleTextClasses}>Add New Subject</DialogTitle> <DialogDescription className={descriptionTextClasses}>Enter details and assign an initial teacher.</DialogDescription> </DialogHeader>
            <form onSubmit={handleAddSubmit} className="space-y-6 py-1">
              <SubjectFormFields formData={formData} onFormChange={handleFormChange} onSelectChange={handleSelectChange} departmentsList={departments} teachersList={teachers} isLoadingDeps={isLoadingDeps} isEdit={false}/>
      ...
      {error && !isAddDialogOpen && ( <Alert variant="destructive"> <AlertTriangleIcon className="h-4 w-4" /> <AlertTitle>Error</AlertTitle> <AlertDescription>{error}</AlertDescription> </Alert> )}
        ...
        <Table>
          <TableHeader>
            <TableRow className="border-zinc-200/80 dark:border-zinc-700/80">
              ...
              <TableHead className={`${titleTextClasses} font-semibold hidden lg:table-cell`}>Initial Teacher</TableHead> {/* ✨ New Column */}
            ...
            {isLoading ? (
              Array.from({ length: 5 }).map((_, index) => (
                <TableRow key={`skeleton-${index}`} className="border-zinc-200/50 dark:border-zinc-800/50">
                  ...
                  <TableCell className="hidden lg:table-cell"><Skeleton className="h-5 w-32 rounded" /></TableCell> {/* ✨ Skeleton for new column */}
              ...
              <TableRow key={subject.id} className="border-zinc-200/50 dark:border-zinc-800/50 hover:bg-zinc-500/5 dark:hover:bg-white/5">
                ...
                {/* ✨ Displaying the first assigned teacher (if any) ✨ */}
                <TableCell className={`${descriptionTextClasses} hidden lg:table-cell`}>
                  {subject.taughtBy && subject.taughtBy.length > 0 
                    ? `${subject.taughtBy[0].staff.user.firstName} ${subject.taughtBy[0].staff.user.lastName}`
      ...
      {/* TODO: Add Edit Subject Dialog, similar to Add Dialog but for updating */}
    </div>
  );
}