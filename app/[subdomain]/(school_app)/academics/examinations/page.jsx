...
const initialExamFormData = {
  ...
  name: '',
  termId: '',
  academicYearId: '', // Helper for the form
};
...
const initialExamScheduleFormData = {
  ...
  startTime: '09:00',
  endTime: '10:00',
  room: '', // Switched from roomId to room based on schema
  maxMarks: '100',
};
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4 p-1">
      ...
      <div>
        ...
        <Select name="academicYearId" value={formData.academicYearId || ''} onValueChange={(value) => onSelectChange('academicYearId', value)} disabled={isLoadingDeps}>
          ...
          <SelectContent>{isLoadingDeps ? <SelectItem value="loading" disabled>Loading...</SelectItem> : academicYears.length === 0 ? <SelectItem value="no-years" disabled>No academic years found</SelectItem> : academicYears.map(year => <SelectItem key={year.id} value={year.id}>{year.name}</SelectItem>)}</SelectContent>
        </Select>
      </div>
      <div>
        ...
        <Select name="termId" value={formData.termId || ''} onValueChange={(value) => onSelectChange('termId', value)} disabled={isLoadingDeps || !formData.academicYearId}>
          ...
          <SelectContent>{!formData.academicYearId && <SelectItem value="select-year-first" disabled>Select Academic Year First</SelectItem>}{formData.academicYearId && filteredTerms.length === 0 && <SelectItem value="no-terms" disabled>No terms for this year</SelectItem>}{filteredTerms.map(term => <SelectItem key={term.id} value={term.id}>{term.name}</SelectItem>)}</SelectContent>
        </Select>
      </div>
    </div>
  );
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4 max-h-[70vh] overflow-y-auto p-1 custom-scrollbar">
      <div className="sm:col-span-2">
        ...
        <Select name="examId" value={formData.examId || ''} onValueChange={(value) => onSelectChange('examId', value)} disabled={isLoadingDeps}>
          ...
          <SelectContent>{isLoadingDeps ? <SelectItem value="loading" disabled>Loading...</SelectItem> : examsList.length === 0 ? <SelectItem value="no-exams" disabled>No exams available</SelectItem> : examsList.map(exam => <SelectItem key={exam.id} value={exam.id}>{`${exam.name} (${exam.term?.academicYear?.name})`}</SelectItem>)}</SelectContent>
        </Select>
      </div>
      <div>
        ...
        <Select name="classId" value={formData.classId || ''} onValueChange={(value) => onSelectChange('classId', value)} disabled={isLoadingDeps}>
          ...
          <SelectContent>{isLoadingDeps ? <SelectItem value="loading" disabled>Loading...</SelectItem> : classesList.length === 0 ? <SelectItem value="no-classes" disabled>No classes available</SelectItem> : classesList.map(c => <SelectItem key={c.id} value={c.id}>{c.name}</SelectItem>)}</SelectContent>
        </Select>
      </div>
      <div>
        ...
        <Select name="subjectId" value={formData.subjectId || ''} onValueChange={(value) => onSelectChange('subjectId', value)} disabled={isLoadingDeps || !formData.classId}>
          ...
          <SelectContent>{!formData.classId && <SelectItem value="select-class-first" disabled>Select Class First</SelectItem>}{formData.classId && filteredSubjects.length === 0 && <SelectItem value="no-subjects" disabled>No subjects for this class</SelectItem>}{filteredSubjects.map(subject => <SelectItem key={subject.id} value={subject.id}>{subject.name}</SelectItem>)}</SelectContent>
        </Select>
      </div>
      <div>
...
export default function ManageExaminationsPage() {
  ...
  const fetchAllData = useCallback(async () => {
    ...
    try {
      const [examsRes, schedulesRes, yearsRes, subjectsRes, classesRes] = await Promise.all([
      ...
      setRooms([]); // Placeholder until rooms API is built
    } catch (err) {
  ...
  const openEditExamDialog = (exam) => {
    const allTerms = academicYears.flatMap(ay => ay.terms || []);
    const term = allTerms.find(t => t.id === exam.termId);
    setEditingExam(exam);
  ...
  const getAcademicYearName = useCallback((termId) => { const term = allTerms.find(t => t.id === termId); return term ? (academicYears.find(y => y.id === term.academicYearId)?.name || 'N/A') : 'N/A'; }, [allTerms, academicYears]);
  ...
  const getExamNameAndContext = useCallback((id) => { const exam = exams.find(e => e.id === id); if (!exam) return 'N/A'; return `${exam.name} (${exam.term?.academicYear?.name})`; }, [exams]);
  const getRoomName = useCallback((roomName) => roomName || 'N/A', []); // ✨ Corrected to handle string
                ...
                <TableCell className="text-right">
                  <div className="flex justify-end gap-1 md:gap-2">
                    ...
                    <Link href={`/${subdomain}/academics/examinations/schedules/${schedule.id}/grades`} passHref>
                      <Button asChild variant="outline" size="icon" className={`${outlineButtonClasses} h-8 w-8`} title="Enter Grades"> 
                        <span className="cursor-pointer flex items-center justify-center"><CheckSquare className="h-4 w-4" /></span>
                      </Button>
                    </Link>
