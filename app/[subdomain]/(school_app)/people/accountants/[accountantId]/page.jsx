...
export default function AccountantProfilePage() {
  ...
  const { subdomain, accountantId } = params || {};
  const [data, setData] = useState(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [editOpen, setEditOpen] = useState(false);
  const [form, setForm] = useState({ firstName:"", lastName:"", email:"", phoneNumber:"", jobTitle:"" });
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const res = await fetch(`/api/schools/${schoolId}/people/accountants/${accountantId}`);
        if (!res.ok) {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Dialog open={editOpen} onOpenChange={(o)=>{ setEditOpen(o); if (o && data) setForm({ firstName:data.firstName||"", lastName:data.lastName||"", email:data.email||"", phoneNumber:data.phoneNumber||"", jobTitle:data.jobTitle||"" }); }}>
          ...
          <AlertDialog>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>This will permanently remove this accountant. This action cannot be undone.</AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                ...
                <AlertDialogAction asChild>
                  <Button variant="destructive" disabled={deleting} onClick={async ()=>{
                    ...
                    setDeleting(true);
                    try {
                      const res = await fetch(`/api/schools/${schoolId}/people/accountants/${accountantId}`, { method:'DELETE' });
                      if (!res.ok) { const j = await res.json().catch(()=>({error:'Failed'})); throw new Error(j.error||'Delete failed'); }
                      toast.success('Deleted');
                      router.push(`/${subdomain}/people/accountants`);
                    } catch (err) {
          ...
          <Button variant="outline" size="sm" onClick={() => router.push(`/${subdomain}/people/accountants`)}>Back</Button>
        </div>
      </div>
      <Separator />
      <div className="grid gap-6 md:grid-cols-3">
        ...
        <Card className="p-4 space-y-2">
          <h2 className="text-sm font-medium">Payments Processed</h2>
          <p className="text-2xl font-semibold">{fin.payments.count}</p>
          <p className="text-xs text-muted-foreground">Total Amount: {formatCurrency(fin.payments.totalAmount)}</p>
        </Card>
        <Card className="p-4 space-y-2">
          <h2 className="text-sm font-medium">Expenses Recorded</h2>
          <p className="text-2xl font-semibold">{fin.expenses.count}</p>
          <p className="text-xs text-muted-foreground">Total Amount: {formatCurrency(fin.expenses.totalAmount)}</p>
        </Card>
      </div>
      <div className="grid gap-6 md:grid-cols-2">
        <Card className="p-4">
          ...
          <ul className="space-y-2 text-xs">
            {fin.recentPayments.length === 0 && <li className="text-muted-foreground">None</li>}
            {fin.recentPayments.map(p => (
              <li key={p.id} className="flex justify-between">
                <span>{new Date(p.paymentDate || p.createdAt).toLocaleDateString()}</span>
                <span className="font-medium">{(typeof p.amount === 'number' ? p.amount : 0).toLocaleString()}</span>
              </li>
            ))}
          </ul>
        </Card>
        <Card className="p-4">
          ...
          <ul className="space-y-2 text-xs">
            {fin.recentExpenses.length === 0 && <li className="text-muted-foreground">None</li>}
            {fin.recentExpenses.map(e => (
              <li key={e.id} className="flex justify-between">
                <span>{new Date(e.date || e.createdAt).toLocaleDateString()}</span>
                <span className="font-medium">{(typeof e.amount === 'number' ? e.amount : 0).toLocaleString()}</span>
              </li>
            ))}
          </ul>
        </Card>
