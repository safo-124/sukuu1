...
export default function AccountantsPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const res = await fetch(`/api/schools/${schoolId}/people/accountants`);
        if (!res.ok) throw new Error("Failed");
        const json = await res.json();
        if (!ignore) setData(json.data || []);
      } catch (e) {
        if (!ignore) setError(e.message || "Failed to load accountants");
      } finally {
  ...
  const filtered = data.filter(acc => {
    ...
    return (
      ...
      acc.staffIdNumber?.toLowerCase().includes(q)
    );
  });
  ...
  return (
    <div className="space-y-6 p-4 md:p-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">Accountants</h1>
          <p className="text-sm text-muted-foreground">Manage school finance staff</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
              <Button size="sm" variant="outline">Add Accountant</Button>
            </DialogTrigger>
            <DialogContent className="max-w-lg">
              <DialogHeader>
                <DialogTitle>New Accountant</DialogTitle>
              </DialogHeader>
              <form
                onSubmit={async (e) => {
                  e.preventDefault();
                  ...
                  try {
                    ...
                    setOpen(false);
                    setForm({ firstName:"", lastName:"", email:"", password:"", staffIdNumber:"", jobTitle:"Accountant" });
                  } catch (err) {
      ...
      {!loading && !error && (
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          ...
          {filtered.length === 0 && (
            <p className="text-sm text-muted-foreground col-span-full">No accountants found.</p>
          )}
        </div>
      )}
    </div>
