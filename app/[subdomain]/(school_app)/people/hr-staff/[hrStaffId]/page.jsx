...
export default function HRStaffProfilePage() {
  ...
  const { subdomain, hrStaffId } = params || {};
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <AlertDialog>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                <AlertDialogTitle>Delete HR Staff</AlertDialogTitle>
                <AlertDialogDescription>This will permanently remove this HR staff member. This action cannot be undone.</AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                ...
                <AlertDialogAction asChild>
                  <Button variant="destructive" disabled={deleting} onClick={async ()=>{
                    ...
                    setDeleting(true);
                    try {
                      const res = await fetch(`/api/schools/${schoolId}/people/hr-staff/${hrStaffId}`, { method:'DELETE' });
                      if (!res.ok) { const j = await res.json().catch(()=>({error:'Failed'})); throw new Error(j.error||'Delete failed'); }
                      toast.success('Deleted');
                      router.push(`/${subdomain}/people/hr-staff`);
                    } catch (err) {
          ...
          <Button variant="outline" size="sm" onClick={() => router.push(`/${subdomain}/people/hr-staff`)}>Back</Button>
        </div>
      </div>
      <Separator />
      <div className="grid gap-6 md:grid-cols-3">
        ...
        <Card className="p-4 space-y-2">
          <h2 className="text-sm font-medium">Pending Leave Applications</h2>
          <p className="text-2xl font-semibold">{data.stats?.pendingLeaves ?? 0}</p>
          <p className="text-xs text-muted-foreground">Across the school</p>
        </Card>
        <Card className="p-4 space-y-2">
          <h2 className="text-sm font-medium">Role</h2>
          <p className="text-xs font-medium uppercase tracking-wide">{data.role}</p>
        </Card>
      </div>
    </div>
  );
