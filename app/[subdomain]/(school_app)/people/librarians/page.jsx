...
export default function LibrariansPage() {
  ...
  async function load() {
    ...
    try {
      const [libRes, depRes] = await Promise.all([
        fetch(`/api/schools/${schoolId}/people/librarians`),
      ...
      if (!libRes.ok) throw new Error(libs?.error || 'Failed to load librarians')
      setRows(Array.isArray(libs.data) ? libs.data : [])
  ...
  async function save() {
    try {
      ...
      const url = isEdit ? `/api/schools/${schoolId}/people/librarians/${form.id}` : `/api/schools/${schoolId}/people/librarians`
  ...
  async function del(id) {
    ...
    try {
      const res = await fetch(`/api/schools/${schoolId}/people/librarians/${id}`, { method: 'DELETE' })
  ...
  return (
    <div className="p-4 sm:p-6 space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-semibold">Librarians</h1>
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild>
            <Button onClick={openCreate}>New Librarian</Button>
          </DialogTrigger>
          <DialogContent className="sm:max-w-xl">
            <DialogHeader>
              <DialogTitle>{form.id ? 'Edit Librarian' : 'New Librarian'}</DialogTitle>
            </DialogHeader>
        ...
        <Table>
          ...
          <TableBody>
            {loading ? (
              ...
              <TableRow><TableCell colSpan={6}>No librarians yet.</TableCell></TableRow>
            ) : rows.map(r => (
              <TableRow key={r.id}>
