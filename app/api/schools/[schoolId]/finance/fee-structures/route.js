...
export async function POST(request, { params }) {
  ...
  try {
    ...
    const { name, description, amount, frequency, academicYearId, classId } = validation.data;

    // Validate linked entities belong to the school
    const [academicYear, _class] = await Promise.all([
    ...
    // Handle unique constraint violation (P2002) for name, academicYearId, classId
    if (error.code === 'P2002') {
