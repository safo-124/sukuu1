...
import { updateLibrarianSchema } from "@/validators/academics.validators";
...
export async function GET(request, { params }) {
  try {
    ...
    console.error('GET /librarians/[id] error', err);
    return NextResponse.json({ error: 'Failed to load librarian profile' }, { status: 500 });
  }
}
...
export async function PATCH(request, { params }) {
  try {
    ...
    const parsed = updateLibrarianSchema.safeParse(json);
    if (!parsed.success) return NextResponse.json({ error: parsed.error.flatten() }, { status: 400 });

    const data = parsed.data;

    const result = await prisma.$transaction(async (tx) => {
      const staff = await tx.staff.findFirst({ where: { id: librarianId, schoolId }, include: { user: true } });
      ...
      if (Object.keys(staffUpdate).length) {
        await tx.staff.update({ where: { id: librarianId }, data: staffUpdate });
      }
    ...
    if (!result) return NextResponse.json({ error: 'Librarian not found' }, { status: 404 });
    ...
    console.error('PATCH /librarians/[id] error', err);
    if (err.message === 'NOT_FOUND') return NextResponse.json({ error: 'Librarian not found' }, { status: 404 });
...
export async function DELETE(request, { params }) {
  try {
    ...
    const staff = await prisma.staff.findFirst({ where: { id: librarianId, schoolId }, include: { user: true } });
    if (!staff) return NextResponse.json({ error: 'Librarian not found' }, { status: 404 });

    await prisma.$transaction([
      prisma.staff.delete({ where: { id: librarianId } }),
      prisma.user.delete({ where: { id: staff.userId } })
    ]);

    return NextResponse.json({ success: true });
  } catch (err) {
    console.error('DELETE /librarians/[id] error', err);
