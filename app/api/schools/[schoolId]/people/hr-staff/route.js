...
import { createHRStaffSchema } from '@/validators/academics.validators';
...
export async function GET(request, { params }) {
  try {
    ...
    const staffRows = await prisma.staff.findMany({
      where: { schoolId, user: { role: 'HR_MANAGER' } },
      orderBy: { createdAt: 'desc' },
      include: { user: { select: { id: true, firstName: true, lastName: true, email: true, phoneNumber: true, profilePictureUrl: true, role: true } }, department: { select: { id: true, name: true } } }
    });

    const hrStaff = staffRows.map(s => ({
      id: s.id,
      userId: s.userId,
    ...
    return NextResponse.json({ data: hrStaff });
  } catch (err) {
...
export async function POST(request, { params }) {
  try {
    ...
    const parsed = createHRStaffSchema.safeParse(json);
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      let hashed = password;
      try { const bcrypt = await import('bcryptjs').catch(()=>null); if (bcrypt?.hashSync) hashed = bcrypt.hashSync(password,10); } catch {}

      const user = await tx.user.create({
        data: { email, hashedPassword: hashed, firstName, lastName, phoneNumber: phoneNumber || null, profilePictureUrl: profilePictureUrl || null, role: 'HR_MANAGER', schoolId },
        select: { id: true, firstName: true, lastName: true, email: true, phoneNumber: true, profilePictureUrl: true, role: true }
      });

      const staff = await tx.staff.create({
        data: { userId: user.id, schoolId, staffIdNumber, jobTitle: jobTitle || 'HR Manager', qualification: qualification || null, departmentId: departmentId || null },
    ...
    const response = { id: result.staff.id, userId: result.user.id, firstName: result.user.firstName, lastName: result.user.lastName, email: result.user.email, phoneNumber: result.user.phoneNumber, profilePictureUrl: result.user.profilePictureUrl, role: result.user.role, staffIdNumber: result.staff.staffIdNumber, jobTitle: result.staff.jobTitle, qualification: result.staff.qualification, departmentId: result.staff.departmentId, createdAt: result.staff.createdAt };

    return NextResponse.json({ data: response }, { status: 201 });
  } catch (err) {
    console.error('POST /hr-staff error', err);
    const msg = (err?.message === 'Email already in use' || err?.message === 'Staff ID number already exists') ? err.message : 'Failed to create HR staff';
