...
import { createHostelWardenSchema } from "@/validators/academics.validators";
...
export async function GET(request, { params }) {
  try {
    ...
    const staffRows = await prisma.staff.findMany({
      where: { schoolId, user: { role: 'HOSTEL_WARDEN' } },
    ...
    return NextResponse.json({ data: wardens });
  } catch (err) {
...
export async function POST(request, { params }) {
  try {
    ...
    const parsed = createHostelWardenSchema.safeParse(json);
    if (!parsed.success) {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const staff = await tx.staff.create({
        data: {
          ...
          schoolId,
          staffIdNumber,
          jobTitle: jobTitle || 'Hostel Warden',
    ...
    console.error("POST /wardens error", err);
