...
import { updateHostelWardenSchema } from "@/validators/academics.validators";
...
export async function GET(request, { params }) {
  try {
    ...
    console.error('GET /wardens/[id] error', err);
    return NextResponse.json({ error: 'Failed to load warden profile' }, { status: 500 });
  }
}
...
export async function PATCH(request, { params }) {
  try {
    ...
    const parsed = updateHostelWardenSchema.safeParse(json);
    if (!parsed.success) return NextResponse.json({ error: parsed.error.flatten() }, { status: 400 });

    const data = parsed.data;

    const result = await prisma.$transaction(async (tx) => {
      const staff = await tx.staff.findFirst({ where: { id: wardenId, schoolId }, include: { user: true } });
      ...
      if (Object.keys(staffUpdate).length) {
        await tx.staff.update({ where: { id: wardenId }, data: staffUpdate });
      }
    ...
    if (!result) return NextResponse.json({ error: 'Warden not found' }, { status: 404 });
    ...
    console.error('PATCH /wardens/[id] error', err);
    if (err.message === 'NOT_FOUND') return NextResponse.json({ error: 'Warden not found' }, { status: 404 });
...
export async function DELETE(request, { params }) {
  try {
    ...
    const staff = await prisma.staff.findFirst({ where: { id: wardenId, schoolId }, include: { user: true } });
    if (!staff) return NextResponse.json({ error: 'Warden not found' }, { status: 404 });

    await prisma.$transaction([
      prisma.staff.delete({ where: { id: wardenId } }),
      prisma.user.delete({ where: { id: staff.userId } })
    ]);

    return NextResponse.json({ success: true });
  } catch (err) {
    console.error('DELETE /wardens/[id] error', err);
