...
export async function GET(request, { params }) {
  ...
  try {
    const [books, borrowed, borrowers] = await Promise.all([
    ...
    const borrowedCount = borrowed?._sum?.quantity ?? 0;
    const activeBorrowers = borrowers.length;

    return NextResponse.json({ books, borrowed: borrowedCount, activeBorrowers }, { status: 200 });
  } catch (err) {
