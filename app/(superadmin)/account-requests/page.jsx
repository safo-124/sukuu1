...
const statusColor = (s) => {
  switch (s) {
    case 'NEW': return 'bg-sky-600/80 text-white border-sky-500/50';
    case 'REVIEWING': return 'bg-amber-600/80 text-white border-amber-500/50';
    case 'APPROVED': return 'bg-emerald-600/80 text-white border-emerald-500/50';
    case 'REJECTED': return 'bg-rose-600/80 text-white border-rose-500/50';
    case 'CONTACTED': return 'bg-violet-600/80 text-white border-violet-500/50';
    default: return 'bg-zinc-700 text-white border-zinc-600';
  }
};
...
export default function AccountRequestsPage() {
  ...
  const load = async () => {
    setLoading(true);
    const res = await fetch(`/api/superadmin/account-requests?${paramsString}`);
  ...
  const updateStatus = async (id, newStatus) => {
    ...
    setSavingId(id);
    await fetch('/api/superadmin/account-requests', {
      method: 'PATCH',
  ...
  const exportCsv = () => {
    const url = `/api/superadmin/account-requests?${paramsString}${paramsString ? '&' : ''}format=csv`;
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-3">
        ...
        <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search name/email/org" className="bg-zinc-900 border border-zinc-700 rounded px-3 py-2" />
  ...
  <button onClick={exportCsv} className="px-3 py-2 border border-zinc-700 rounded">Export CSV</button>
      ...
      {loading ? <div>Loading...</div> : (
        <div className="space-y-4">
          {data.map((r) => (
            <div key={r.id} className="rounded-lg border border-white/15 bg-white/10 p-4 backdrop-blur-sm dark:border-white/10 dark:bg-zinc-900/40">
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
                <div>
                  <div className="text-base font-medium">{r.organization || 'Individual'} — {r.requesterName}</div>
          ...
          {data.length === 0 && (
            <div className="text-center text-zinc-400">No account requests found.</div>
          )}
        </div>
      )}
    </div>
